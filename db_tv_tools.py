    ...
    for show_s in db.list_shows():
        ...
        for season_d in show_d["seasons"]:
            ...
            for episode_d in season_d["episodes"]:
                ...
                episode_d["full_path"] = os.path.join(
                    tv_root, show_s, season_d["folder"], episode_d["file"])
...
def find_ep_data(key, data):
    PRINT.info(f"showing all eps where [{key} == {data}]")
    found = 0
    eps = __flatten_eps()
    for ep in eps:
        if ep[key] == data:
            PRINT.info(ep["file"])
            found += 1
    PRINT.info(f"scan done, found [{found}] eps")
...
def check_nfo():
    ...
    for show in shows:
        if show.startswith("@"):  # Diskstation
            continue
        if show in skiplist:
            PRINT.info(f"in skiplist: [{show}]")
            continue
        if tvtool.has_nfo(show):
            if tvtool.nfo_to_imdb(show):
                PRINT.info("{} has tvshow.nfo".format(show))
            else:
                PRINT.warning("{} has non-imdb tvshow.nfo".format(show))
...
def se_upper():
    ...
    for ep in __flatten_eps():
        ...
        if ep["se"].startswith("s"):
            ...
            PRINT.info(f"upper: {ep['se'].upper()}")
            need_save = True
    if need_save:
        db.save()
...
def check_status():
    PRINT.info("confirming episode status flags...")
    need_save = False
    for ep in __flatten_eps():
        if ep["status"] == "deleted":
            if ftool.is_existing_file(ep['full_path']):
                PRINT.warning(f"not deleted: [{ep['full_path']}]")
                si = ep["season_index"]
                ei = ep["episode_index"]
                show_d = db.data(ep["show"])
            ...
            if not ftool.is_existing_file(ep['full_path']):
                PRINT.info(f"found deleted: [{ep['full_path']}]")
                si = ep["season_index"]
                ei = ep["episode_index"]
                show_d = db.data(ep["show"])
...
def omdb_update():
    PRINT.info("trying to omdb-search for missing data")
    save_db = False
    success_count = 0
    for show_s in db.list_shows():
        if in_skip_list(show_s):
            PRINT.info(f"skipping {show_s}, in skip list...")
            continue
        need_update = False
        show_d = db.data(show_s)
...
def tvmaze_update():
    PRINT.info("trying to tvmaze-search for missing data")
    save_db = False
    success_count = 0
    for show_s in db.list_shows():
        if in_skip_list(show_s):
        ...
        for season_d in show_d["seasons"]:
            ...
            if not season_d["tvmaze"]:
                tvmaze_result = tvtool.tvmaze_search_season(
                    show_d, season_d["folder"])
                if tvmaze_result:
    ...
    for ep in __flatten_eps():
        ...
        if not ep['subs']['sv']:
            ...
            if sv_srt_file:
                ...
                db.update(ep["show"], show_d, key=None)
                PRINT.info(f"found [{sv_srt_file}]")
                save_db = True
        if not ep['subs']['en']:
            ...
            if en_srt_file:
                ...
                db.update(ep["show"], show_d, key=None)
                PRINT.info(f"found [{en_srt_file}]")
                save_db = True
    PRINT.info("done!")
    if save_db:
        db.save()
...
def omdb_force_update(show_s):
    success_count = 0
    PRINT.info("force-updating omdb-data for {}".format(show_s))
    show_d = db.data(show_s)
    nfo_imdb = tvtool.nfo_to_imdb(show_d)
    if nfo_imdb:
        if show_d["imdb"] != nfo_imdb:
...
parser.add_argument('--show', "-s", type=str, dest="show_s",
                    default=None, help='show to process')
parser.add_argument('--key', "-k", type=str, dest="key",
                    default=None, help='key')
parser.add_argument('--data', "-d", type=str,
                    dest="data", default=None, help='daa')
